...
describe('windows', () => {
	...
	it('marks windows as not complete when complete button is clicked then user removes an item', async () => {
		...
		expect(store.livingSpaceFabric.livingSpaceWindows.complete).toBe(true);
	
		await user.click(screen.getByTestId('windows_remove_0'));
	
		expect(store.livingSpaceFabric.livingSpaceWindows.complete).toBe(false);
		expect(screen.getByRole("button", { name: "Mark section as complete" })).not.toBeNull();
	});
	...
	it('should navigate to the living space fabric overview page when return to overview is clicked', async () => {
		await renderSuspended(Windows);
