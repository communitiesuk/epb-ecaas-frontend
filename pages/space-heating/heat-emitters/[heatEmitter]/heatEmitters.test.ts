...
describe("Heat emitters", () => {
	...
	describe("Validation", () => {
		...
		test("allows the heat emitters section to be marked complete when there are no emitters", async () => {
			store.$patch({
				spaceHeating: {
					heatEmitters: {
		...
		test("does not allow the heat emitters section to be marked complete when an emitter is incomplete", async () => {
			...
			const wetDistributionWithIncompleteEmitter: HeatEmittingData = {
				...wetDistributionSystem,
			...
			store.$patch({
				spaceHeating: {
					heatEmitters: {
						data: [{
							data: wetDistributionWithIncompleteEmitter,
							complete: false,
						}],
					},
				},
		...
		test("allows the heat emitters section to be marked complete when the emitter is complete", async () => {
			...
			const wetDistributionWithCompleteEmitter: HeatEmittingData = {
				...wetDistributionSystem,
			...
			store.$patch({
				spaceHeating: {
					heatEmitters: {
						data: [{
							data: wetDistributionWithCompleteEmitter,
							complete: false,
						}],
					},
				},
		...
		test("does not allow the heat emitters section to be marked complete when one of multiple emitters is incomplete", async () => {
			const completeRadiator = {
				id: "emitter1",
				name: "Complete radiator",
				typeOfHeatEmitter: "radiator" as const,
				productReference: "product-ref-123",
				length: unitValue(1500, millimetre),
				numOfRadiators: 2,
		...
		test("shows an error when trying to complete the section with an incomplete emitter", async () => {
			const incompleteRadiator = {
				id: "emitter1",
				name: "Radiator",
				typeOfHeatEmitter: "radiator" as const,
		...
		test("emitter validation error links to the incomplete emitter from the error summary", async () => {
			const incompleteRadiator = {
				id: "emitter1",
				name: "Radiator",
				typeOfHeatEmitter: "radiator" as const,
			...
			expect(
				errorSummary.querySelector('a[href="#emitter-emitter1"]'),
			).toBeDefined();
		});

		test("shows heat emitter form, incompatible energy source and emitter validation errors in error summary when all have invalid fields", async () => {
			const wetDistributionSystemWithMissingFields: Partial<HeatEmittingData> = {
				id: "1234",
				name: "Wet Distribution System 1",
				typeOfHeatEmitter: "wetDistributionSystem",
			...
			expect(errorSummary.textContent).toContain("Design flow rate is required.");
			expect(errorSummary.textContent).toContain(
				"Complete all fields in Fan Coil before marking the heat emitters section as complete.",
			);
			expect(errorSummary.textContent).toContain("This product uses LPG (Liquid petroleum gas) - bulk which hasn't been added as an energy source for this dwelling.");
		});

		test("shows all emitter validation errors in error summary when they have invalid fields", async () => {
			const wetDistributionSystemWithIncompleteEmitters: HeatEmittingData = {
				...wetDistributionSystem,
				emitters: [
					{
						id: "fan_coil",
			...
			store.$patch({
				spaceHeating: {
					heatEmitters: {
						data: [{ data: wetDistributionSystemWithIncompleteEmitters }],
					},
				},
			});
			...
			expect(errorSummary.textContent).toContain(
				"Complete all fields in Emitter 1 before marking the heat emitters section as complete.",
			);

			expect(errorSummary.textContent).toContain(
				"Complete all fields in Emitter 2 before marking the heat emitters section as complete.",
			);
		});

		test("doesn't save an invalid radiator to store", async () => {
			const incompleteRadiator = {
				id: "1234",
