...
const heatInterfaceUnitData = useItemToEdit("interface", store.spaceHeating.heatGeneration.heatInterfaceUnit.data);
const model: Ref<HeatInterfaceUnitData> = ref(heatInterfaceUnitData!);

const saveForm = (fields: HeatInterfaceUnitData) => {
	store.$patch((state) => {
		const { heatInterfaceUnit } = state.spaceHeating.heatGeneration;
