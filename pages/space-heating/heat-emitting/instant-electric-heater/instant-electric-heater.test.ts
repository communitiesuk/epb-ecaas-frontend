...
describe("instantElectricHeater", () => {
	...
	test("data is saved to store state when form is valid", async () => {
		...
		const { data } = store.spaceHeating.heatEmitting.instantElectricHeater;
	...
	test("updated form data is automatically saved to store", async () => {
		...
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.name).toBe("Updated Instant electric heater 1");
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.ratedPower).toBe(5);
	});
	...
	test("partial form data is saved automatically with default name to store when adding new heater", async () => {
		...
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.name).toBe("Instant electric heater");
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.ratedPower).toBe(5);
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.convectiveType).toBe("Air heating (convectors, fan coils etc.)");
	});
	...
	test("creates a new heater automatically with given name", async () => {
		...
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.name).toBe("Heater 1");
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.ratedPower).toBeUndefined();
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[0]?.data.convectiveType).toBeUndefined();
	});
	...
	test("updated form data is automatically saved to the correct store object when there are multiple instant electric heaters added", async () => {
		...
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[1]?.data.name).toBe("Updated Instant electric heater 2");
		expect(store.spaceHeating.heatEmitting.instantElectricHeater.data[1]?.data.ratedPower).toBe(1);
	});
});
