...
const electricStorageHeaterData = useItemToEdit("heater", store.spaceHeating.heatEmitting.electricStorageHeater.data);
const model: Ref<ElectricStorageHeaterData> = ref(electricStorageHeaterData!);

const saveForm = (fields: ElectricStorageHeaterData) => {
	store.$patch((state) => {
		const { electricStorageHeater } = state.spaceHeating.heatEmitting;
