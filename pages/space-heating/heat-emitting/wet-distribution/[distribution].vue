...
const wetDistributionStoreData = store.spaceHeating.heatEmitting.wetDistribution.data;
const index = getStoreIndex(wetDistributionStoreData);
const wetDistributionData = useItemToEdit(
	"distribution",
	store.spaceHeating.heatEmitting.wetDistribution.data,
);
...
const saveForm = (fields: WetDistributionData) => {
	...
	store.$patch((state) => {
		const { wetDistribution } = state.spaceHeating.heatEmitting;
...
autoSaveElementForm<WetDistributionData>({
	model,
	storeData: store.spaceHeating.heatEmitting.wetDistribution,
	defaultName: "Wet distribution",
	onPatch: (state, newData, index) => {
		// we only support radiators
		newData.data.typeOfSpaceHeater = "radiator";
		state.spaceHeating.heatEmitting.wetDistribution.data[index] = newData;
		state.spaceHeating.heatEmitting.wetDistribution.complete = false;
	},
});
