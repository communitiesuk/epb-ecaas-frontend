...
const applianceData = useItemToEdit("combustion", store.infiltrationAndVentilation.combustionAppliances["open_gas_kitchen_stove"].data);
const model: Ref<CombustionApplianceData> = ref(applianceData!);

const saveForm = (fields: CombustionApplianceData) => {
	store.$patch((state) => {
		const { open_gas_kitchen_stove: openGasKitchenStove } = state.infiltrationAndVentilation.combustionAppliances;
