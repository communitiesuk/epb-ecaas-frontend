...
describe('mechanical ventilation form', () => {
	...
	it('data is saved to store state when form is valid and typeOfMechanicalVentilationOptions_mvhr is mvhr', async () => {
		...
		await user.type(screen.getByTestId('thermalInsulationConductivityOfDuctwork'), '10');
		await user.click(screen.getByTestId('surfaceReflectivity_reflective'));
		await user.click(screen.getByRole('button'));

		const { data, complete } = store.infiltrationAndVentilation.InfilAndVentMechanicalVentilation;
		expect(complete).toBe(true);
		expect(data.mechanicalVentilationObjects?.[0]).toEqual(mechanicalVentilation1);
		expect(navigateToMock).toHaveBeenCalledWith('/infiltration-and-ventilation/mechanical-ventilation');
	});
	...
	it('data is saved to store state when form is valid and typeOfMechanicalVentilationOptions is not mvhr', async () => {
		...
		await user.click(screen.getByTestId('typeOfMechanicalVentilationOptions_intermittent'));
		...
		await waitFor(() => {
			const { data, complete } = store.infiltrationAndVentilation.InfilAndVentMechanicalVentilation;
			expect(complete).toBe(true);
			expect(data.mechanicalVentilationObjects?.[0]).toEqual(mechanicalVentilation2);
			expect(navigateToMock).toHaveBeenCalledWith('/infiltration-and-ventilation/mechanical-ventilation');
		});
	});

	it('data is saved to correct object in store state when form is valid', async () => {
		store.$patch({infiltrationAndVentilation: {
			InfilAndVentMechanicalVentilation: {
				data: {
					mechanicalVentilationObjects: [mechanicalVentilation1, mechanicalVentilation2]
				}
			}
		}});
		...
		const { data } = store.infiltrationAndVentilation.InfilAndVentMechanicalVentilation;
	...
	it('form is prepopulated when data exists in state', async() => {
		...
		expect((await screen.findByTestId('name') as HTMLInputElement).value).toBe('Mechanical name 1');
		expect(((await screen.findByTestId('typeOfMechanicalVentilationOptions_mvhr')) as HTMLInputElement).checked).toBe(true);
		expect(((await screen.findByTestId('controlForSupplyAirflow_load')) as HTMLInputElement).checked).toBe(true);
		...
		expect((await screen.findByTestId('externalDiameterOfDuctwork') as HTMLInputElement).value).toBe('1000');
		expect((await screen.findByTestId('insulationThickness') as HTMLInputElement).value).toBe('100');
		expect((await screen.findByTestId('lengthOfDucwork') as HTMLInputElement).value).toBe('100');
		expect((await screen.findByTestId('thermalInsulationConductivityOfDuctwork') as HTMLInputElement).value).toBe('10');
		expect(((await screen.findByTestId('surfaceReflectivity_reflective')) as HTMLInputElement).checked).toBe(true);
	...
	it('required error messages are displayed when empty form is submitted', async () => {
		...
		const mvhrErrorIds: string[] = ['mvhrLocation_error',
	...
	it('error summary is displayed when an invalid form in submitted', async () => {
		...
		expect((await screen.findByTestId('mechanicalVentilationErrorSummary'))).toBeDefined();
	});
