import { getByRole, getByTestId, screen } from '@testing-library/vue';
...
describe('mechanical ventilation overview', () => {
  ...
  it('should only remove the mechanical ventilation object that is clicked', async ()=> {
    store.$patch({
      infiltrationAndVentilation: {
        InfilAndVentMechanicalVentilation: {
          data: {
            mechanicalVentilationObjects: [mechanicalVentilation1, mechanicalVentilation2, mechanicalVentilation3]
          }
        }
      }
    })
  ...
  it('should duplicate mechanical ventilation object when duplicate button is clicked', async() => {
    ...
    await renderSuspended(MechanicalVentilationOverview)
        ...
        await renderSuspended(MechanicalVentilationOverview);
        await userEvent.click(screen.getByTestId('mechanicalVentilation_duplicate_0'));
        await userEvent.click(screen.getByTestId('mechanicalVentilation_duplicate_0'));
        await userEvent.click(screen.getByTestId('mechanicalVentilation_duplicate_2'));
        await userEvent.click(screen.getByTestId('mechanicalVentilation_duplicate_2'));
    
        expect(screen.queryAllByTestId('mechanicalVentilation_item').length).toBe(6);
