...
const groundFloorSummary: SummarySection = {
	...
	data: groundFloorData.map(x => {
		const edgeInsulationWidth = x.data.typeOfGroundFloor === FloorType.Slab_edge_insulation ? (typeof x.data.edgeInsulationWidth === "number" ? x.data.edgeInsulationWidth : x.data.edgeInsulationWidth.amount) : undefined;
		...
		return {
			...
			"Edge insulation type": x.data.typeOfGroundFloor === FloorType.Slab_edge_insulation ? displayCamelToSentenceCase(x.data.edgeInsulationType) : undefined,
			"Edge insulation width": edgeInsulationWidth ? `${edgeInsulationWidth} ${centimetre.suffix}` : undefined,
			"Edge insulation thermal resistance": x.data.typeOfGroundFloor === FloorType.Slab_edge_insulation ? `${x.data.edgeInsulationThermalResistance} ${squareMeterKelvinPerWatt.suffix}` : undefined,
			"Height of the floor upper surface": x.data.typeOfGroundFloor === FloorType.Suspended_floor ? `${x.data.heightOfFloorUpperSurface} ${millimetre.suffix}` : undefined,
			"Thermal resistance of insulation on base of underfloor space": x.data.typeOfGroundFloor === FloorType.Suspended_floor ? `${x.data.underfloorSpaceThermalResistance} ${squareMeterKelvinPerWatt.suffix}` : undefined,
			"Thermal transmittance of walls above ground": x.data.typeOfGroundFloor === FloorType.Suspended_floor ? `${x.data.thermalTransmittanceOfWallsAboveGround} ${wattsPerSquareMeterKelvin.suffix}` : undefined,
			"Area of ventilation openings per perimeter": x.data.typeOfGroundFloor === FloorType.Suspended_floor ? `${x.data.ventilationOpeningsArea} ${millimetresSquarePerMetre.suffix}` : undefined,
			"Wind shielding factor": x.data.typeOfGroundFloor === FloorType.Suspended_floor ? x.data.windShieldingFactor : undefined,
