...
const windowSummary: SummarySection = {
	...
	data: windowData.map(x => {
		return {
			...
			"Mid height of the air flow path for openable part 3": x.numberOpenableParts === '3' || x.numberOpenableParts === '4' ? x.midHeightOpenablePart3 : undefined,
			"Mid height of the air flow path for openable part 4": x.numberOpenableParts === '4' ? x.midHeightOpenablePart4 : undefined,
			"Overhang depth": 'overhangDepth' in x ? typeof x.overhangDepth !== 'number' && x.overhangDepth.amount : undefined,
			"Overhang distance from glass": 'overhangDistance' in x ? typeof x.overhangDistance !== 'number' && x.overhangDistance.amount : undefined,
			"Side fin right depth": 'sideFinRightDepth' in x ? typeof x.sideFinRightDepth !== 'number' && x.sideFinRightDepth.amount : undefined,
			"Side fin right distance from glass": 'sideFinRightDistance' in x ? typeof x.sideFinRightDistance !== 'number' && x.sideFinRightDistance.amount : undefined,
			"Side fin left depth": 'sideFinLeftDepth' in x ? typeof x.sideFinLeftDepth !== 'number' && x.sideFinLeftDepth.amount : undefined,
			"Side fin left distance from glass": 'sideFinLeftDistance' in x ? typeof x.sideFinLeftDistance !== 'number' && x.sideFinLeftDistance.amount : undefined,
