...
const roofData = useItemToEdit('roof', store.dwellingFabric.dwellingSpaceCeilingsAndRoofs.dwellingSpaceUnheatedPitchedRoofs?.data);
const model: Ref<RoofData> = ref(roofData!);

const saveForm = (fields: RoofData) => {
	store.$patch((state) => {
		const {dwellingSpaceUnheatedPitchedRoofs} = state.dwellingFabric.dwellingSpaceCeilingsAndRoofs;
		...
		dwellingSpaceUnheatedPitchedRoofs.complete = false;
		saveToList(roof, dwellingSpaceUnheatedPitchedRoofs);
	});
