...
describe("heatInterfaceUnit", () => {
	...
	test("data is saved to store state when form is valid", async () => {
		...
		const { data } = store.heatingAndCoolingSystems.heatGeneration.heatInterfaceUnit;
	...
	test("navigates to hot water outlets page when valid form is completed", async () => {
		...
		expect(navigateToMock).toHaveBeenCalledWith("/heating-and-cooling-systems/heat-generation");
	});
});