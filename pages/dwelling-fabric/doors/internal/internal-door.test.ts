...
describe("internal door", () => {
  ...
  describe("when type of internal door is heated space", () => {
    ...
    it("form is prepopulated when data exists in state", async () => {
      ...
      expect(
        (
          await screen.findByTestId("typeOfInternalDoor_heatedSpace")
        ).hasAttribute("checked")
      ).toBe(true);
      expect(
        (
          await screen.findByTestId(`associatedItemId_${internalWall.id}`)
        ).hasAttribute("checked")
      ).toBe(true);
      ...
      expect(
        (await screen.findByTestId<HTMLInputElement>("surfaceArea")).value
      ).toBe("5");
      expect(
        (await screen.findByTestId("kappaValue_50000")).hasAttribute("checked")
      ).toBe(true);
      expect(
        (await screen.findByTestId("massDistributionClass_I")).hasAttribute(
          "checked"
        )
      ).toBe(true);
    });
    ...
    it("requires additional fields when heated space is selected", async () => {
      ...
      expect(
        await screen.findByTestId("massDistributionClass_error")
      ).toBeDefined();
    });
  });
  ...
  describe("when type of internal door is unheated space", () => {
    it("data is saved to store state when form is valid", async () => {
      ...
      await user.type(
        screen.getByTestId("thermalResistanceOfAdjacentUnheatedSpace"),
        "0"
      );
      await user.tab();
      await user.click(screen.getByTestId("saveAndComplete"));
    ...
    it("form is prepopulated when data exists in state", async () => {
      ...
      expect(
        (
          await screen.findByTestId("typeOfInternalDoor_unheatedSpace")
        ).hasAttribute("checked")
      ).toBe(true);
      expect(
        (
    ...
    it("requires additional fields when heated space is selected", async () => {
      ...
      expect(
        await screen.findByTestId(
          "thermalResistanceOfAdjacentUnheatedSpace_error"
        )
      ).toBeDefined();
    });
  });
