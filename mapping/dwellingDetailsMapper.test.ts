...
import { mapApplianceData, mapDistantShadingData, mapDwellingDetailsData, mapExternalFactorsData, mapGeneralSpecificationsData } from './dwellingDetailsMapper';
...
describe('dwelling details mapper', () => {
	...
	it('maps appliances input state to FHS input request', () => {
		...
		console.log('Resolved store state:', resolveState(store.$state));

		// Act
		const fhsInputData = mapApplianceData(resolveState(store.$state));
