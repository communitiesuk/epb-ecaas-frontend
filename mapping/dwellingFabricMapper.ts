...
export function mapFloorData(
  ...
  const groundFloorData: { [key: string]: BuildingElementGround }[] =
    dwellingSpaceGroundFloor.map((x) => {
      ...
      switch (x.typeOfGroundFloor) {
        ...
        default:
          throw new Error(
            `Unexpected ground floor type: ${
              (x as { typeOfGroundFloor: string }).typeOfGroundFloor
            }`
          );
      }
...
export function mapWallData(
  ...
  const { dwellingSpaceExternalGlazedDoor } =
    state.dwellingFabric.dwellingSpaceDoors;
  const { dwellingSpaceExternalUnglazedDoor } =
    state.dwellingFabric.dwellingSpaceDoors;
  ...
  const externalWallData: { [key: string]: SchemaBuildingElement }[] =
    dwellingSpaceExternalWall?.map((x) => {
      ...
      const netSurfaceArea = calculateResolvedNetSurfaceArea(x, [
  ...
  const externalGlazedDoorData: { [key: string]: SchemaBuildingElement }[] =
    dwellingSpaceExternalGlazedDoor.map((x) => {
      ...
      const glazedDoor = {
        ...
        frame_area_fraction: calculateFrameToOpeningRatio(
          x.openingToFrameRatio
        ),
...
export function mapThermalBridgingData(
  ...
  const pointThermalBridgesData: Record<string, SchemaThermalBridgingPoint>[] =
