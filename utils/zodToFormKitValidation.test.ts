...
describe("convert zod types to formkit validation strings", () => {
	...
	describe("zodUnit schemas", () => {
		it("converts zodUnit schema to amount-based rules", () => {
			expect(zodTypeAsFormKitValidation(addConstraints(zodUnit("length"), { min: 0, max: 100 }))).toBe("required | zodUnitNumber | zodUnitMin:0 | zodUnitMax:100");
		});

		it("converts optional zodUnit schema without required rule", () => {
			expect(zodTypeAsFormKitValidation(addConstraints(zodUnit("length"), { min: 0, max: 100 }).optional())).toBe("zodUnitNumber | zodUnitMin:0 | zodUnitMax:100");
		});

		it("converts exclusive zodUnit bounds to viable FormKit range", () => {
			expect(zodTypeAsFormKitValidation(addConstraints(zodUnit("length"), { gt: 0, lt: 10 }))).toBe("required | zodUnitNumber | zodUnitGreaterThan:0 | zodUnitLessThan:10");
		});
	});
});